...
export default class UserController {
  ...
  public static async getSchool(ctx: BaseContext, next: () => void) {
    ...
    if (school) {
      ...
      ctx.state.message =
        "The school you are trying to retrieve doesn't exist in the db";
    }
    await next();
  }
  ...
  public static async updateSchool(ctx: BaseContext, next: () => void) {
    ...
    if (errors.length > 0) {
      ...
      ctx.status = httpStatus.BAD_REQUEST;
      ctx.state.message =
        "The school you are trying to update doesn't exist in the db";
    } else if (
      await schoolRepository.findOne({
        id: Not(Equal(schoolToBeUpdated.id)),
        name: schoolToBeUpdated.name,
      })
    ) {
  ...
  public static async deleteSchool(ctx: BaseContext, next: () => void) {
    ...
    const schoolToRemove: School = await schoolRepository.findOne(
      +ctx.params.schoolId || 0
    );
    if (!schoolToRemove) {
      // return a BAD REQUEST status code and error message
