...
export class NotificationService {
  ...
  public async sendAndCleanUp(
    ...
    const resultsFromProvider = await this.getPushResultsFromProviders(
    ...
    if (ticketsNotRegistered.length) {
      tokensNotRegistered = tokens.filter((notificationToken) =>
        ticketsNotRegistered.includes(notificationToken.token)
      );
  ...
  public getErrorsFromProviders(
    allReceipts: ExpoPushReceipt[]
  ): IExpoPushResult {
    const ticketsNotRegistered = [];
    const ticketsWithErrors = [];
    ...
    const receiptsWithError = receipts.filter(
      (receipt) =>
        receipt.status === CONSTANTS.ERROR_MESSAGES.PUSH_NOTIFICATION_ERROR
    );
  ...
  public getPushResultsFromExpo(
    allTickets: ExpoPushTicket[]
  ): IExpoPushResult & { validTickets: INotificationTicket[] } {
    const ticketsNotRegistered = [];
    const ticketsWithErrors = [];
    const validTickets = [];
    ...
    for (let i = 0; i < tickets.length; i++) {
      ...
      const hasError =
        status === CONSTANTS.ERROR_MESSAGES.PUSH_NOTIFICATION_ERROR;
      ...
      const isDeviceNotRegistered =
        details &&
        details.error === CONSTANTS.ERROR_MESSAGES.DEVICE_NOT_REGISTERED;
      ...
      const message =
        details && details.error
          ? this.getErrorMessage(details.error)
          : 'Unknown error.';
  ...
  public async getPushResultsFromProviders(
    tickets: INotificationTicket[]
  ...
  public async removeTokensNotRegistered(
    ...
    const tokensToRemove = allTokens.filter((notificationToken) =>
      tokensNotRegistered.includes(notificationToken.token)
    );

    return Promise.all(
      tokensToRemove.map(async ({ id }) => this.repository.delete(id))
    );
  }
